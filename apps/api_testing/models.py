...
class ApiRequest(models.Model):
    ...
    project = models.ForeignKey(ApiProject, on_delete=models.CASCADE, related_name='requests',
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 如果有 collection，但没有 project，则从 collection 获取
        if self.collection and not self.project and self.collection.pk:
            try:
                self.project = self.collection.project
            except Exception:
                pass
    ...
    def save(self, *args, **kwargs):
        # 如果有 collection，但没有 project，则从 collection 获取
        if self.collection and not self.project:
            try:
                if hasattr(self.collection, 'project') and self.collection.project:
                    self.project = self.collection.project
            except Exception:
                pass  # 如果出了问题，不阻止保存
