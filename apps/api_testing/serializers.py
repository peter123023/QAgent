...
class ApiCollectionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'order', 'children', 'requests', 'created_at', 'updated_at'
        ]
        read_only_fields = ['created_at', 'updated_at']
    ...
    def get_requests(self, obj):
        # 两种获取方式：一是通过 collection 关联的，二是通过 project 关联的但没有 collection 的
        # 不过这里我们只获取直接关联到这个 collection 的请求
        requests = obj.requests.all().order_by('order', 'created_at')
        return ApiRequestSerializer(requests, many=True).data
...
class ApiRequestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'post_request_script', 'assertions', 'project', 'collection', 'order', 'created_by',
            'created_at', 'updated_at'
        ]
        read_only_fields = ['created_at', 'updated_at']
