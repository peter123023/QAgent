...
class ApiRequestFilter(df_filters.FilterSet):
    """API请求自定义过滤器"""
    collection__isnull = df_filters.BooleanFilter(field_name='collection', lookup_expr='isnull')
    
    class Meta:
        model = ApiRequest
        fields = ['project', 'collection', 'method', 'request_type']
...
class ApiRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # 查询两种情况：有 project 字段，或者通过 collection 关联
...
class TestExecutionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def generate_allure_report(self, request, pk=None):
        ...
        try:
            ...
            allure_cmd = base_dir / 'tools' / 'allure' / 'bin' / allure_executable
