...
from .models import (
    ...
    GenerationConfig, AIModelService, AICaseTemplate, AIWriterConfig, AIReviewerConfig
)
...
class AICaseTemplateViewSet(viewsets.ModelViewSet):
    """AI用例模板（含版本）"""
    queryset = AICaseTemplate.objects.all()
    serializer_class = AICaseTemplateSerializer
    ...
    def diff(self, request):
        ...
        if not source_id or not target_id:
            return Response({'error': 'source_id 和 target_id 为必填'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            source = self.get_queryset().get(id=source_id)
            target = self.get_queryset().get(id=target_id)
        except AICaseTemplate.DoesNotExist:
            return Response({'error': '模板版本不存在'}, status=status.HTTP_404_NOT_FOUND)

        diff_lines = difflib.unified_diff(
            source.content.splitlines(),
            target.content.splitlines(),
            fromfile=f"{source.name}:{source.version}",
            tofile=f"{target.name}:{target.version}",
            lineterm=''
        )

        return Response({
            'source': {'id': source.id, 'name': source.name, 'version': source.version},
            'target': {'id': target.id, 'name': target.name, 'version': target.version},
            'diff': '\n'.join(diff_lines)
        }, status=status.HTTP_200_OK)


class AIWriterConfigViewSet(viewsets.ModelViewSet):
    """AI用例编写配置"""
    queryset = AIWriterConfig.objects.all()
    serializer_class = AIWriterConfigSerializer

    def get_queryset(self):
        queryset = super().get_queryset().prefetch_related('models_selected', 'template')
        if self.request.user.is_authenticated:
            queryset = queryset.filter(created_by=self.request.user)
        return queryset.order_by('-created_at')
    ...
    def enable(self, request, pk=None):
        ...
        return Response({'message': '编写配置已启用', 'id': instance.id}, status=status.HTTP_200_OK)

    @action(detail=True, methods=['post'])
    def test_generation(self, request, pk=None):
        """采样测试生成"""
        config = self.get_object()
        requirement_text = request.data.get('requirement_text', '请生成登录功能测试用例')
        model_id = request.data.get('model_id')

        selected_models = config.models_selected.filter(is_active=True)
        if model_id:
            selected_models = selected_models.filter(id=model_id)

        model = selected_models.first()
        if not model:
            return Response({'error': '当前编写配置未选择可用模型，请先在设置中配置并启用模型'}, status=status.HTTP_400_BAD_REQUEST)

        template_content = config.template.content if config.template else ''
        prompt = config.prompt_content or (
            "你是测试用例编写专家，请输出结构化测试用例。"
        )
        full_prompt = f"{template_content}\n\n{prompt}\n\n需求:\n{requirement_text}".strip()

        messages = [
            {"role": "system", "content": "你是资深测试专家"},
        ...
        try:
            ...
            asyncio.set_event_loop(loop)
            result = loop.run_until_complete(
                AIModelService.call_openai_compatible_api(model, messages)
            )
...
class AIReviewerConfigViewSet(viewsets.ModelViewSet):
    """AI用例评审配置"""
    queryset = AIReviewerConfig.objects.all()
    serializer_class = AIReviewerConfigSerializer
    parser_classes = [MultiPartParser, FormParser, JSONParser]

    def get_queryset(self):
        queryset = super().get_queryset().select_related('review_model')
        if self.request.user.is_authenticated:
            queryset = queryset.filter(created_by=self.request.user)
        return queryset.order_by('-created_at')
    ...
    def enable(self, request, pk=None):
        ...
        instance.is_active = True
        instance.save(update_fields=['is_active'])
        return Response({'message': '评审配置已启用', 'id': instance.id}, status=status.HTTP_200_OK)

    @action(detail=False, methods=['get'])
    def best_prompt(self, request):
        """返回默认的评审提示词"""
        import os
        ...
        try:
            ...
            reviewer_prompt = """请按JSON输出评分、问题清单、改进建议。"""
        
        return Response({'prompt': reviewer_prompt}, status=status.HTTP_200_OK)

    @action(detail=True, methods=['post'])
    def simulate_review(self, request, pk=None):
        """模拟评审：上传用例文件并返回评分、缺陷和diff"""
        config = self.get_object()
        if not config.review_model or not config.review_model.is_active:
            return Response({'error': '当前评审配置未绑定可用评审模型，请先在设置中配置并启用模型'}, status=status.HTTP_400_BAD_REQUEST)

        uploaded_files = request.FILES.getlist('files')
        raw_case_text = request.data.get('case_text', '')
        file_text_blocks = []
        for f in uploaded_files:
            try:
                file_text_blocks.append(f.read().decode('utf-8', errors='ignore'))
            except Exception:
                file_text_blocks.append('')

        combined_text = '\n\n'.join([raw_case_text] + file_text_blocks).strip()
        if not combined_text:
            return Response({'error': '请上传至少一个用例文件或填写用例文本'}, status=status.HTTP_400_BAD_REQUEST)

        dimensions = config.review_dimensions or [
            {'name': '完整性', 'weight': 25},
            {'name': '正确性', 'weight': 25},
            {'name': '可维护性', 'weight': 25},
            {'name': '业务覆盖率', 'weight': 25},
        ]
        dim_text = ', '.join([f"{d.get('name')}({d.get('weight', 0)}%)" for d in dimensions])
        strategy = config.review_strategy or {}

        review_prompt = config.prompt_content or "请按JSON输出评分、问题清单、改进建议。"
        user_prompt = (
            f"{review_prompt}\n"
            f"评审维度: {dim_text}\n"
            f"策略: {json.dumps(strategy, ensure_ascii=False)}\n"
            f"请返回JSON结构: {{score_distribution:{{}}, defects:[], revised_text:\"\"}}\n"
            f"待评审用例:\n{combined_text}"
        )
        messages = [
            {"role": "system", "content": "你是测试用例评审专家，请严格输出JSON"},
        ...
        try:
            ...
            asyncio.set_event_loop(loop)
            result = loop.run_until_complete(
                AIModelService.call_openai_compatible_api(config.review_model, messages)
            )
            content = result.get('choices', [{}])[0].get('message', {}).get('content', '') or ''

            score_distribution = {}
            defects = []
            revised_text = combined_text

            try:
                parsed = json.loads(content)
                score_distribution = parsed.get('score_distribution', {}) or {}
                defects = parsed.get('defects', []) or []
                revised_text = parsed.get('revised_text') or combined_text
            except Exception:
                defects = [line for line in content.splitlines() if line.strip()][:20]
                for dim in dimensions:
                    score_distribution[dim.get('name', '维度')] = max(
                        0, min(100, int(config.confidence_threshold * 100))
                    )
...
class ConfigStatusViewSet(viewsets.ViewSet):
    ...
    def check(self, request):
        ...
        try:
            ...
            writer_config = AIWriterConfig.get_active_config(request.user)
            reviewer_config = AIReviewerConfig.get_active_config(request.user)
            ...
            response_data = {
                ...
                'writer_case_config': {
                    'configured': writer_config is not None,
                    'enabled': writer_config.is_active if writer_config else False,
                    'name': writer_config.name if writer_config else None,
                    'id': writer_config.id if writer_config else None
                },
                'reviewer_case_config': {
                    'configured': reviewer_config is not None,
                    'enabled': reviewer_config.is_active if reviewer_config else False,
                    'name': reviewer_config.name if reviewer_config else None,
                    'id': reviewer_config.id if reviewer_config else None
                },
                'model_pool': {
                    'writer_active_count': ai_model_configs.filter(role='writer', is_active=True).count(),
                    'reviewer_active_count': ai_model_configs.filter(role='reviewer', is_active=True).count()
                }
            }
...
class DashboardViewSet(viewsets.ViewSet):
    ...
    def dashboard_stats(self, request):
        ...
        try:
            # 需求相关统计
            from django.db.models import Count
            requirement_doc_count = RequirementDocument.objects.count()
            ai_generated_count = GeneratedTestCase.objects.count()
            pending_review_count = GeneratedTestCase.objects.filter(status='pending_review').count()
            reviewed_count = GeneratedTestCase.objects.filter(status='reviewed').count()
