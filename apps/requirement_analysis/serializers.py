...
from .models import (
    ...
    GenerationConfig, AICaseTemplate, AIWriterConfig, AIReviewerConfig
)
...
class AICaseTemplateSerializer(serializers.ModelSerializer):
    created_by_name = serializers.CharField(source='created_by.username', read_only=True)

    class Meta:
        model = AICaseTemplate
        fields = [
            'id', 'name', 'version', 'content', 'changelog', 'is_published',
            'created_by', 'created_by_name', 'created_at', 'updated_at'
        ]
...
class AIWriterConfigSerializer(serializers.ModelSerializer):
    created_by_name = serializers.CharField(source='created_by.username', read_only=True)
    template_name = serializers.CharField(source='template.name', read_only=True)
    template_version = serializers.CharField(source='template.version', read_only=True)
    models_selected_detail = AIModelConfigSerializer(source='models_selected', many=True, read_only=True)

    class Meta:
        model = AIWriterConfig
        fields = [
            'id', 'name', 'template', 'template_name', 'template_version',
            'models_selected', 'models_selected_detail',
            'prompt_content', 'prompt_ab_versions', 'prompt_variables',
            'generation_options', 'output_formats', 'post_hooks',
            'is_active', 'created_by', 'created_by_name', 'created_at', 'updated_at'
        ]
        read_only_fields = ['created_by', 'created_by_name', 'created_at', 'updated_at']

    def validate_models_selected(self, models_selected):
        for model in models_selected:
            if model.role not in ['writer', 'other']:
                raise serializers.ValidationError('编写配置仅可选择编写类模型')
        return models_selected

    def create(self, validated_data):
        models_selected = validated_data.pop('models_selected', [])
        user = self.context['request'].user
        if user.is_authenticated:
            validated_data['created_by'] = user
        else:
        ...
        if models_selected:
            instance.models_selected.set(models_selected)
        return instance

    def update(self, instance, validated_data):
        models_selected = validated_data.pop('models_selected', None)
...
class AIReviewerConfigSerializer(serializers.ModelSerializer):
    created_by_name = serializers.CharField(source='created_by.username', read_only=True)
    review_model_name = serializers.CharField(source='review_model.name', read_only=True)
    review_model_type = serializers.CharField(source='review_model.model_type', read_only=True)

    class Meta:
        model = AIReviewerConfig
        fields = [
            'id', 'name', 'review_model', 'review_model_name', 'review_model_type',
            'confidence_threshold', 'prompt_content', 'review_dimensions',
            'review_strategy', 'best_prompt_snapshot', 'is_active',
            'created_by', 'created_by_name', 'created_at', 'updated_at'
        ]
        read_only_fields = ['created_by', 'created_by_name', 'created_at', 'updated_at']

    def validate_review_model(self, review_model):
        if review_model and review_model.role != 'reviewer':
            raise serializers.ValidationError('评审配置仅可选择评审类模型')
        return review_model
