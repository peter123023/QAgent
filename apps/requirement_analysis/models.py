...
class AICaseTemplate(models.Model):
    """AI用例模板（支持版本）"""
    name = models.CharField(max_length=100, verbose_name='模板名称')
    version = models.CharField(max_length=30, verbose_name='版本号')
    content = models.TextField(verbose_name='模板内容')
    changelog = models.TextField(blank=True, verbose_name='版本说明')
    ...
    class Meta:
        db_table = 'ai_case_templates'
        verbose_name = 'AI用例模板'
        verbose_name_plural = 'AI用例模板'
...
class AIWriterConfig(models.Model):
    """AI用例编写配置"""
    RETRY_STRATEGY_CHOICES = [
        ('none', '不重试'),
        ('fixed', '固定间隔'),
        ('exponential', '指数退避'),
    ]
    ...
    name = models.CharField(max_length=100, verbose_name='配置名称', default='默认编写配置')
    template = models.ForeignKey(
        AICaseTemplate, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='writer_configs', verbose_name='模板版本'
    )
    models_selected = models.ManyToManyField(
        AIModelConfig, blank=True, related_name='writer_configs_selected', verbose_name='模型选择'
    )
    prompt_content = models.TextField(blank=True, verbose_name='提示词内容')
    prompt_ab_versions = models.JSONField(default=dict, blank=True, verbose_name='Prompt A/B版本')
    prompt_variables = models.JSONField(default=list, blank=True, verbose_name='Prompt变量')
    generation_options = models.JSONField(default=dict, blank=True, verbose_name='生成高级配置')
    output_formats = models.JSONField(default=list, blank=True, verbose_name='输出格式')
    post_hooks = models.JSONField(default=dict, blank=True, verbose_name='后置钩子')
    is_active = models.BooleanField(default=True, verbose_name='是否启用')
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name='创建者')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='更新时间')
    ...
    class Meta:
        db_table = 'ai_writer_configs'
        verbose_name = 'AI用例编写配置'
        verbose_name_plural = 'AI用例编写配置'
        ordering = ['-created_at']
    ...
    @classmethod
    def get_active_config(cls, user=None):
...
class AIReviewerConfig(models.Model):
    """AI用例评审配置"""
    name = models.CharField(max_length=100, verbose_name='配置名称', default='默认评审配置')
    review_model = models.ForeignKey(
        AIModelConfig, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='reviewer_configs_selected', verbose_name='评审模型'
    )
    confidence_threshold = models.FloatField(default=0.8, verbose_name='置信度阈值')
    prompt_content = models.TextField(blank=True, verbose_name='提示词内容')
    review_dimensions = models.JSONField(default=list, blank=True, verbose_name='评审维度与权重')
    review_strategy = models.JSONField(default=dict, blank=True, verbose_name='评审策略')
    best_prompt_snapshot = models.TextField(blank=True, verbose_name='历史最佳Prompt快照')
    is_active = models.BooleanField(default=True, verbose_name='是否启用')
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name='创建者')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='更新时间')
    ...
    class Meta:
        db_table = 'ai_reviewer_configs'
        verbose_name = 'AI用例评审配置'
        verbose_name_plural = 'AI用例评审配置'
        ordering = ['-created_at']
